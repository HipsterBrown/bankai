...
function preloadTransform () {
  ...
  var base64 = sha256(content)
  var header = `<script nomodule integrity="${base64}">${content}</script>`
  return addToHead(header)
}

function scriptTransform (opts) {
  var hex = opts.hash.hexSlice(16)
  var base64 = 'sha256-' + opts.hash.base64Slice()
  var link = `/${hex}/bundle.js`
  var header = `<script src="${link}" integrity="${base64}" defer></script>`
  return addToHead(header)
}
...
function styleTransform (opts) {
  ...
  var header = `<link rel="preload" as="style" href="${link}" integrity=${base64} onload="this.rel='stylesheet'">`
  return addToHead(header)
}
...
function reloadTransform (opts) {
  ...
  var base64 = 'sha256-' + sha256(bundle)
  var header = `
    <script integrity=${base64}>${bundle}</script>
  `.replace(/\n +/g, '')
  return addToHead(header)
}
